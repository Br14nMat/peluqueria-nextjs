...
export default function Reservations() {
    ...
    const client = useAppSelector((state) => state.users.selectedClient);

    useEffect(() => {
        if (currentUser?.token) {
            dispatch(fetchReservationsByClient({ clientId: client?.id || '', token: currentUser.token }));
        }
    ...
    return (
        <div className="p-5 m-5 grid grid-cols-1 md:grid-cols-2 gap-6">
            {reservations.length === 0 ? (
                <p className="text-center text-azulOscuro">No hay reservas disponibles.</p>
            ) : (
